...
public class Sistema implements Serializable {
     ...
     * @throws CodigoRepetidoDocente          se um código de um docente lido já
     *                                        estiver cadastrado.
     ...
     * @throws SiglaRepetidaVeiculo    se um veículo a ser inserido possuir sigla já
     *                                 utilizada.
     * @throws TipoVeiculoDesconhecido se um veículo tiver um tipo desconhecido.
     * @throws InputMismatchException  se houver erro de formatação.
     */
    public void carregaArquivoVeiculos(Scanner infile)
            throws SiglaRepetidaVeiculo, TipoVeiculoDesconhecido, InputMismatchException {
        infile.nextLine(); // Ignora primeira linha
        infile.useDelimiter(";");
     ...
     * @throws CodigoDocenteIndefinido          se o código de um docente não tiver
     *                                          sido definido na planilha de
     *                                          docentes.
     * @throws SiglaVeiculoPublicacaoIndefinida se a sigla de um veículo não tiver
     *                                          sido definida na planilha de
     *                                          veículos.
     * @throws InputMismatchException           se houver erro de formatação.
     */
    public void carregaArquivoPublicacao(Scanner infile)
            throws CodigoDocenteIndefinido, SiglaVeiculoPublicacaoIndefinida, InputMismatchException {
        infile.nextLine(); // Ignora primeira linha
        infile.useDelimiter(";");
        ...
        while (infile.hasNext()) {
            ...
            Publicacao pub = new Publicacao(ano, titulo, numero, volume, local, paginaIni, paginaFim, autoresObj,
                    veiculo);
            for (String itAutores : autores) {
                long cod = Long.parseLong(itAutores.replaceAll("\\s+", ""));
                Docente doc = this.docentesCadastrados.get(cod);
     ...
     * @throws QualisDesconhecidoVeiculo se houver Qualis desconhecida para um
     *                                   veículo.
     * @throws SiglaIndefinidaVeiculo    se houver sigla de veículo não definida na
     *                                   planilha de veículos.
     * @throws InputMismatchException    se houver erro de formatação.
     */
    public void carregaArquivoQualis(Scanner infile)
            throws SiglaIndefinidaVeiculo, QualisDesconhecidoVeiculo, InputMismatchException {
        infile.nextLine(); // Ignora primeira linha
        infile.useDelimiter(";");
        int ano;
        String siglaVeiculo;
     ...
     * @throws QualisDesconhecidoRegra        se houver Qualis desconhecida para uma
     *                                        regra.
     * @throws InputMismatchException         se houver erro de formatação.
     * @throws ArrayIndexOutOfBoundsException se houver erro de formatação.
     */
        ...
        for (Publicacao itPub : this.publicacoes) {
            ...
            if (qualis != null) {
                outfile.write(itPub.getAno() + ";" + itPub.getVeiculo().getSigla() + ";" + itPub.getVeiculo().getNome()
                        + ";" + qualis.getNivel() + ";"
                        + String.format("%.3f", itPub.getVeiculo().getImpacto()).replace('.', ',') + ";"
                        + itPub.getTitulo() + ";");
                int i = 0;
